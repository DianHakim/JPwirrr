    ...
    public function run(): void
    {
        ...
        foreach ($dates as $date) {
            if (rand(0, 1) == 0) {
                ...
                $trsCode = 'TRX' . $date->format('Ymd') . '-' . str_pad($nextNumber, 3, '0', STR_PAD_LEFT);

                // Diskon random
                $discountType = ['none', 'percent', 'nominal'][rand(0, 2)];
                $discountPercent = $discountType === 'percent' ? rand(5, 20) : 0;
                $discountNominal = $discountType === 'nominal' ? rand(5000, 20000) : 0;
                ...
                $trx = Transaction::create([
                    ...
                    // ⚡⚡ DISKON LAMA (BIAR STRUK LAMA JALAN) ⚡⚡
                    'trs_discount' => $discountNominal, // ← TAMBAHKAN INI
                ...
                $totalAfterDiscount = max($subtotal - $discountNominal, 0);

                // Cash random >= total untuk testing kembalian
                $cash = $totalAfterDiscount + rand(0, 20000);
